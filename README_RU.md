...
В `ppr_profiles.inc` записаны 55 уникальных PPR layout и 89 точных комбинаций
`FW + retail/testkit/devkit` от 1.00 до 11.40, включая 6.02 и 9.05. Одинаковые
между target варианты ссылаются на один layout, но runtime всё равно требует
наличия точной target-записи. В отдельном `kmb_range_profiles.inc` находятся 32
проверенных KMB layout от 4.00 до 10.60. `make profiles` пересобирает обе
таблицы из организованного архива `PPR_ROOT` (`../../mp4` по умолчанию),
где каталоги первого уровня называются `retail`, `devkit` и `testkit`.
`make verify` независимо извлекает и проверяет каждый профиль по этим ELF.
Положение источника принимается только при совпадении release-маркера внутри
embedded AArch64 ELF; явный target-маркер также обязан совпадать с каталогом.
Ошибочно разложенный образ вызывает fail-closed ошибку инвентаризации.
...
- `a53_ppr_install_fast.elf` — та же установка с динамическим ускорением
  phase5 в дополнение к fast transport defaults;
...
Fast installer также динамически находит kernel globals `tick`, `tick_sbt` и
`hz` по их точному соотношению значений в читаемом KDATA. Требуется ровно одно
совпадение. Непосредственно перед каждым MP4 kick значение `hz` временно
уменьшается в десять раз и восстанавливается сразу после перехода динамически
найденного mailbox state в phase5. Readback, cleanup и восстановление через
`atexit` обязательны; ошибка поиска, записи или восстановления прерывает
payload. Таким образом глобальная clock-переменная меняется только в коротком
окне перехода, а прошивочно-зависимые адреса clock-полей не используются.
Locator поддерживает как ранний порядок `hz, tick, tick_sbt` из 2.50, так и
поздний разнесённый layout. Поиск mailbox принимает расстояние 4 или 8 байт
между `state` и `flags` и при наличии сверяет state со значением из ioctl.
Validator PPR layout игнорирует только два служебных traversal-бита, которые
`mp4_show_layout_info()` выставляет после печати G6/SRAM частей записи. На
ранних прошивках точная таблица может читаться до выставления этих маркеров.
Запись IO-controller DRAM text на любой прошивке принимает оба наблюдавшихся
semantic-значения: `0x0001` и `0x0011`. В обоих случаях сохраняется строгая
проверка геометрии, backing-полей и уникальности записи.
...
профильную stock-инструкцию. Отдельная таблица покрывает все 32 архивных
профиля 4.00–10.60; на 1.x–3.x и 11.x операция завершается как unsupported до
...
Неудачная фаза install выполняется ещё один раз. Если повторная попытка также
не проходит, install возвращает ошибку, но оставляет подтверждённое продвижение
вперёд и не откатывает текущие точки в stock. Следующий install продолжает
точно распознанный прерванный образ прямо к установленному состоянию и не
перезаписывает уже полный исполняемый cave-код под активными caller. Rollback
при uninstall по-прежнему выполняется по фазам и проверяется; если публичные
caller нельзя подтвердить как stock, внутренние hooks намеренно остаются
целыми и payload сообщает `ROLLBACK_REBOOT_REQUIRED`. Быстрый preflight
...
`make verify` проверяет 55 уникальных layout, все 89 target-профилей и их
доступные A53 ELF, три patch-version alias, тринадцать site
...
и заменяющую ветвь для всех 32 KMB-профилей.

`make host-test` проверяет status, обязательный `--idle`, install, ограниченную
повторную forward-попытку, продолжение прерванной установки, uninstall, отказ
при неизвестной инструкции и отказ для неподдерживаемого профиля selector. Отдельный тест
